<template>
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white">إدارة المقالات</h1>
                <p class="text-slate-600 dark:text-slate-400 mt-1">إضافة وتعديل المقالات</p>
            </div>
            <RouterLink :to="{ name: 'articles.create' }" class="btn btn-primary">
                + إضافة مقالة جديدة
            </RouterLink>
        </div>
        ...
        <!-- Table -->
        <div v-else-if="store.articles.length" class="card overflow-hidden">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="article in store.articles" :key="article.id">
                            ...
                            <td>
                                <span :class="['badge', article.is_published
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    ...
                                    <button @click="openDeleteModal(article)"
        ...
        <div v-else class="card p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">لا توجد مقالات</h3>
            <p class="text-slate-600 dark:text-slate-400 mb-4">ابدأ بإضافة مقالة جديدة</p>
            <RouterLink :to="{ name: 'articles.create' }" class="btn btn-primary inline-block">
                + إضافة مقالة جديدة
            </RouterLink>
        </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            <div class="card p-6 max-w-sm w-full">
                <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">حذف المقالة</h3>
...
function confirmDelete() {
    if (selectedItem.value) {
        store.deleteArticle(selectedItem.value.id).then(res => {
